...
class MCTS:
    def __init__(self, state=ConnectState(), exploration_constant = MCTSMeta.EXPLORATION):
    ...
    def select_node(self) -> tuple:
        ...
        while len(node.children) != 0:
            ...
            max_value = max(children, key=lambda n: n.value(self.exploration_constant)).value(self.exploration_constant)
            max_nodes = [n for n in children if n.value(self.exploration_constant) == max_value]
    ...
    # keeping both time and iteration limits, and break when either is hit
    
    def search(self, max_iterations: int = 100000, time_limit: float = 2.0):
        start_time = time.process_time()
        num_rollouts = 0

        while num_rollouts < max_iterations and (time.process_time() - start_time) < time_limit:
